...
interface filtersState { countryName: string, countryId: number, countryCode: string, stdCode: string }
...
export const CountriesPage: React.FC<ICountriesPage> = (props) => {
    const dispatch = useDispatch();
    const grid = useRef(null);
    const state = useRef(null)
    const filtersState = props.filtersState
    const [deletePoppOpen, setDeletePopupOpen] = useState(false);
    const [popupEditorState, setPopupEditorState] = useState({ visible: false, mode: 'ADD', country: { countryName: '', countryId: 0, countryCode: '', stdCode: '' } })
    ...
    const deleteCountry = () => {
        ...
        props.asyncOperationStart(FETCH_COUNTRIES)
    }
    ...
    const btnGridFetch_click = () => {
        const filterValue = [];
        if (filtersState.countryName) {
            filterValue.push(["countryName", "=", filtersState.countryName]);
        }
        if (filtersState.countryCode) {
            filterValue.push(["countryCode", "=", filtersState.countryCode]);
        }
        if (filtersState.stdCode) {
            filterValue.push(["stdCode", "=", filtersState.stdCode]);
        }
    ...
    const btnGridAdd_click = () => {
        setPopupEditorState({ visible: true, mode: 'ADD', country: { countryName: '', countryId: 0, countryCode: '', stdCode: '' } })
    }
    const btnFilterClear_click = () => {
        grid.current.clearFilter();
        props.setFilterState({
            country: { countryName: '', countryId: 0 },
            countryCode: '',
            stdCode: ''
        });
    ...
    const addCountry = (data) => {
        ...
        if (!utils.checkForUniqueValeInGrid("countryName", data.countryName, dataSource, false)) {
    ...
    const updateCountry = (data) => {
        ...
        if (!utils.checkForUniqueValeInGrid("countryName", data.countryName, dataSource, data.id)) {
            errorMessage.push("Country already present");
        }
        if (errorMessage.length === 0) {
    ...
    return (
        <React.Fragment>
            ...
            <div className="pnlFilters">
                <MdaeFormField
                    ...
                    input={<MdaeAutoComplete
                        id="ddlCountry"
                        value={filtersState.countryName}
                ...
                <MdaeFormField
                    label="Country Code"
                    input={<MdaeTextBox
                        value={filtersState.countryCode}
                        onChange={props.handleInpuchnage}
                        name="countryCode" />}
                />
                <MdaeFormField
            ...
            <MdaeDataGrid
                ref={grid}
                dataSource={props.countryList}
                columns={['countryName', 'countryCode', 'stdCode']}
